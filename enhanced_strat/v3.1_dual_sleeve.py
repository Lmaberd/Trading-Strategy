"""
EnhancedStrategy V3.1: Multi-Sleeve Trading Strategy
====================================================
Based on V3 dual sleeve, with helper logic inlined back into the
EnhancedStrategy class so the strategy is self-contained.
"""
...
class EnhancedStrategy(BaseStrategy):
    ...
    @staticmethod
    def _build_analytics_lookup(analytics_df):
        ...
        for ticker in lookup:
            lookup[ticker].sort(key=lambda item: item[0])
        return lookup
    ...
    def _update_universe(self, current_date):
        ...
        print(
            f"  Universe updated ({month_key}): {len(self.universe)} "
            f"stocks selected from {len(scored)} scored"
        )
