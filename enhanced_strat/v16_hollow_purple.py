"""
EnhancedStrategy V16: Hollow Purple
===================================
Based on V14.2 reversed Amihud Parkinson volatility.

Optimisations inherited from V14.2:
...
Additional V16 change:
  - universe selection now includes both the bottom 25% and top 25%
...
class EnhancedStrategy(BaseStrategy):
    ...
    def _select_universe(self, analytics_slice):
        ...
        lower_threshold = scored['composite_score'].quantile(0.25)
        upper_threshold = scored['composite_score'].quantile(0.75)
        return set(
            scored[
                (scored['composite_score'] <= lower_threshold)
                | (scored['composite_score'] >= upper_threshold)
            ]['ticker'].tolist()
        )
    ...
    def _select_universe_from_latest(self, latest_dict):
        ...
        lower_threshold = np.quantile(composite, 0.25)
        upper_threshold = np.quantile(composite, 0.75)

        return {
            t for t, s in zip(valid_tickers, composite)
